...
* **Lip-sync comes from the audio pipeline, not from the avatar.** `main.py`
  ...
  and OPERO's own output. The avatar just consumes that number, so there is no
  second audio path to fall out of sync. The mouth only tracks the level while
  OPERO is *speaking* — during listening the same level drives the aura, so the
...
# to tell speech from room tone. OPERO's own voice arrives as a per-20 ms
...
class HoloAvatar:
    ...
    def _mouth_step(self, dt: float, amp: float, live: bool,
        ...
        """One increment of the jaw. Called once per viseme frame while OPERO
    ...
    def step(self, dt: float, amp: float, speaking: bool = False,
        ...
        # changes when OPERO starts or stops talking jumps the phase by
