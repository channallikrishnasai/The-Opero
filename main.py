...
TOOL_DECLARATIONS = [
    ...
    # tools live in their own action file and are auto-discovered by
        # core.action_loader (see OperaLive.__init__).
    {
        "name": "system_status",
        "description": (
...
class OperaLive:
    ...
    def _on_wake_detected(self) -> None:
        """Called from the detector thread when 'Hey opero' is heard."""
    ...
    def sleep(self, reason: str = "timeout") -> None:
        ...
        self.ui.write_log(f"SYS: Sleeping — {reason}. Say 'Hey opero' to wake me.")
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask OPERO to
    ...
    def _on_text_command(self, text: str):
        ...
        # "Hey opero" or the WAKE NOW button.
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "OPERO").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "OPERO"
            _user_name = ""
        ...
        cfg = dict(
            ...
            # window — OPERO can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Proactive audio: OPERO stays silent when speech isn't addressed
    ...
    async def _flush_pending_vision(self) -> bool:
        ...
        if self._vision_cam_active:
            # Camera: stay busy until OPERO has finished speaking the answer,
    ...
    async def _play_audio(self):
        ...
        try:
            while True:
                ...
                # Drive the HUD waveform and the avatar's mouth from OPERO's
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session and self._awake:
                # Don't interrupt if user spoke recently or OPERO is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                if self.session:
                    ...
                    # has no desktop WAKE button — so it wakes OPERO if asleep.
                    if self._wake_enabled and not self._awake:
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    # until the user says "Hey opero" or taps wake in the UI.
                    if self._wake_enabled:
                        self._ensure_wake_detector()
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        self.ui.write_log("SYS: OPERO online — sleeping. Say 'Hey opero' to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: OPERO online.")
