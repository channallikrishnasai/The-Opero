...
try:
    ...
    notification.notify(title="O.P.E.R.O Reminder", message=message, timeout=15)
    notified = True
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("O.P.E.R.O Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception as e:
        log.debug("%s", e)
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "O.P.E.R.O Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Auto-generated by O.P.E.R.O reminder — do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>O.P.E.R.O Reminder</Description></RegistrationInfo>\n'
    ...
    if result.returncode != 0:
        ...
        log.info(f"[Reminder] ❌ schtasks: {err}")
        return ""  
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.opero.reminder.{task_name}"
...
"""
    ...
    if result.returncode != 0:
        ...
        script_path.unlink(missing_ok=True)
        log.info(f"[Reminder] ❌ launchctl: {result.stderr.strip()}")
        return ""
...
def _schedule_linux(target_dt: datetime, task_name: str,
    ...
    if shutil.which("systemd-run"):
        ...
        if result.returncode == 0:
            return task_name
        log.error(f"[Reminder] ⚠️ systemd-run failed: {result.stderr.strip()}, trying 'at'")
    ...
    if shutil.which("at"):
        ...
        if result.returncode == 0:
            return task_name
        log.info(f"[Reminder] ❌ at: {result.stderr.strip()}")
        return ""

    log.info("[Reminder] ❌ Neither systemd-run nor at found on this Linux system.")
    return ""
...
def reminder(
    ...
    parameters = validate_params(parameters, VALIDATOR, tool_name="reminder")
    ...
    task_name  = f"OPEROReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        script_path.unlink(missing_ok=True)
        log.info(f"[Reminder] ❌ Scheduling exception: {e}")
...
VALIDATOR = {
    "date":    [{"type": str, "required": True, "max_len": 10, "pattern": r"^\d{4}-\d{2}-\d{2}$"}],
    "time":    [{"type": str, "required": True, "max_len": 5, "pattern": r"^\d{2}:\d{2}$"}],
    "message": [{"type": str, "max_len": 500}],
}
