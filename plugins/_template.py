"""
Drop-in OPERO plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — OPERO discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the OperaUI instance — use player.write_log(f"OPERO: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"OPERO: {result_text}")
        except Exception:
            pass
    return result_text
