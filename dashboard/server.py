"""
dashboard/server.py — OPERO Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "OPERO Uploads",
        Path.home() / "Documents" / "OPERO Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'OPERO-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"OPERO Dashboard Port {port}"
        prog_rule  = "OPERO Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="opero_fw_")
        try:
...
def _ensure_certs() -> bool:
    ...
    key_p = certs / "opero.key"
    crt_p = certs / "opero.crt"
    ...
    try:
        ...
        who = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "OPERO Dashboard"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "OPERO"),
        ])
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        certs = BASE_DIR / "config" / "certs"
        return (certs / "opero.key").exists() and (certs / "opero.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in OPERO to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('opero_token','{tok}');
  sessionStorage.setItem('opero_key','{key}');
  localStorage.setItem('opero_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to OPERO…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "opero.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "opero.crt"
