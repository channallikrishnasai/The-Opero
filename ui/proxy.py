"""OperaUI — thread-safe wrapper around MainWindow."""
...
class OperaUI:
    ...
    def push_visemes(self, frames, hop: float, at: float) -> None:
        ...
        for OPERO's own speech. `at` is the wall-clock time the batch begins to
    ...
    def _voice_engine(self):
        return self._win._voice_engine

    @_voice_engine.setter
    def _voice_engine(self, v):
        self._win._voice_engine = v

    def _refresh_voice_engine_btn(self):
        self._win._refresh_voice_engine_btn()
    ...
    def show_quiz(self, topic: str, questions, grade=None) -> None:
        ...
        here. Returning None from it means "OPERO should judge this one", which
